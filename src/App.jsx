...
5. **금지 사항 (반드시 준수):**
    - **모든 주어 표현 금지:**
      - 학생 주어: '학생 A', '그는', '학생은', '본인은', '해당 학생은' 등 절대 사용 금지
      - 교사 주어: '선생님이', '교사가', '담임이', '교과 선생님이' 등 절대 사용 금지
      - 학생 설문에 "선생님이 ~해주셨다"고 작성되어 있어도, 결과문에는 **무주어 형태**로 변환할 것
        - 예시: "선생님이 전시회에 작품을 걸어주셨음" (❌) → "전시회에 작품을 출품하는 기회를 가짐" (✅)
        - 예시: "선생님께서 칭찬해주셨음" (❌) → "수업 중 긍정적인 피드백을 받음" (✅)
...
- 너무 길어지면 가장 중요한 내용만 선별하여 자연스럽게 축약할 것`;
                  ...
                  try {
                    ...
                    if (charCount > 500) {
                      ...
                      // 축약 프롬프트 (원본의 형식 지침 포함)
                      const trimPrompt = `당신은 중학교 1학년 담임 및 교과를 맡은 베테랑 교사입니다.

다음은 학생의 '행동특성 및 종합의견'으로 작성한 텍스트입니다. 이 텍스트를 다음 지침에 따라 450~490자 내로 자연스럽게 축약해주세요.

## 축약 지침:

1. **핵심 내용 유지:**
   - 학생의 성장, 역량, 인성을 보여주는 가장 중요한 내용만 남길 것
   - 구체적인 행동과 에피소드의 본질은 반드시 살릴 것
   - 불필요한 중복 표현과 미사여구는 제거할 것

2. **형식 지침 (반드시 지키기):**
   - **명사형 어미(∼함, ∼임)**로 문장을 마무리할 것
   - **현재형 위주**로 유지할 것 ("현재 보이는", "이번 학년에" 등)
   - 한 문장은 **100자 이내**로 간결하게 유지할 것
   - 모든 문장을 마침표(.)로 명확히 구분할 것

3. **우선순위 판단:**
   - 글자수를 줄이기 어렵다면 가장 후순위로 여겨지는 내용부터 삭제할 것
   - 학생의 학업 태도, 인성, 사회성 중 가장 돋보이는 부분은 반드시 남길 것

4. **금지 사항:**
   - **모든 주어 표현 사용 금지:**
     - 학생 주어: '학생 A', '그는', '학생은', '본인은' 등 금지
     - 교사 주어: '선생님이', '교사가', '담임이' 등 금지
   - 미래 표현("발전이 기대됨", "성장할 것으로 예상됨" 등) 금지
   - 상급학교 관련 표현 금지
